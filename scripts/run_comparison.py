...
def load_selic(
    ...
    candidates = []
    if requested_dir:
        root = Path(requested_dir)
        candidates.extend([
            root / "Selic_252d.xlsx",
            root / "data" / "raw" / "Selic_252d.xlsx",
        ])
    candidates.extend([
        DATA_RAW / "Selic_252d.xlsx",
        BASE_DIR / "data" / "raw" / "Selic_252d.xlsx",
    ])
    ...
    path = next((p for p in unique_candidates if p.exists()), None)
    if path is None:
        raise FileNotFoundError(
            "Arquivo Selic_252d.xlsx não encontrado. Procurado em: "
            + ", ".join(str(p) for p in unique_candidates)
        )

    s = _read_economatica_xlsx(str(path), "Selic")
    if s.empty:
        raise ValueError(f"Arquivo de Selic vazio: {path}")

    # Economatica fornece a taxa anualizada em percentual.
    # Ex.: 10.85 -> 0.1085.
    ...
    if returns_index is not None:
        ...
        if s_period.empty:
            raise ValueError(
                ...
                f"A série disponível vai de {s.index.min()} a {s.index.max()}."
            )
    else:
        s_period = s

    # A série já é anualizada; não se deve elevar a média à potência 252.
    rf = float(s_period.mean())
    logger.info(
        f"  Selic: {rf:.4%} a.a. "
        f"(média da taxa anualizada, {s_period.index.min().date()} a "
...
def _garch_fit_single_asset(args_tuple):
    ...
    try:
        ...
        gres = fitter.fit_single(ret, ticker=ticker)
...
def make_copula_cvar_optimizer(
    ...
    def _optimizer_fn(train_df: pd.DataFrame) -> np.ndarray:
        ...
        try:
            ...
            u_sim = cop.simulate(n_samples=n_sim_effective, seed=win_seed)
            u_sim = np.clip(u_sim, 1e-10, 1 - 1e-10)

            # Convert copula uniforms through the fitted GARCH-EVT marginals.
            # This is the production scenario path: GARCH -> GPD tails -> PPF.
            sim_returns = np.zeros((n_sim_effective, d))
            marginals = {}
            for tkr in tickers:
                from risk.semi_parametric import SemiParametricGARCH_EVT
                # Refit in the parent process so the complete GPD state is
                # available for ppf(); the GARCH result is selected/fitted in
                # the same window and uses the same skewed-t specification.
                fitter_evt = __import__('marginals.garch', fromlist=['GARCHFitter']).GARCHFitter(
                    model_type=garch_model, dist='skewt')
                gres_evt = fitter_evt.fit_single(train_df[tkr], ticker=tkr)
                marginal = SemiParametricGARCH_EVT()
                marginal.initialize_from_garch_result(
                    train_df[tkr], gres_evt, left_quantile=0.05, right_quantile=0.95)
                marginals[tkr] = marginal
            ...
            bnds = [(0.0, 0.35)] * d
            n_restarts_eff = max(int(n_restarts), 1)
