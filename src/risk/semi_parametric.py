...
class SemiParametricGARCH_EVT:
  ...
  def initialize_from_garch_result(self, returns: pd.Series, garch_result,
                                   ...
                                   right_quantile: float = 0.95,
                                   run_gpd: bool = True):
    """Attach an already-fitted GARCH result and build the EVT marginal.

    This keeps the walk-forward GARCH selection in GARCHFitter while using
    the same semi-parametric GPD marginal for scenario generation.
    """
    self._original_returns = returns.copy()
    self.returns = returns.copy()
    self.quantile_left = float(left_quantile)
    self.quantile_right = float(right_quantile)
    self.garch_result = garch_result
    self.garch_model = getattr(garch_result, 'model', None)
    self.conditional_vol = pd.Series(
        np.asarray(garch_result.conditional_vol, dtype=float),
        index=returns.index,
    )
    self.std_residuals = pd.Series(
        np.asarray(garch_result.std_residuals, dtype=float),
        index=returns.index,
    ).dropna()
    self.standardized_residuals = self.std_residuals.copy()
    self._select_thresholds_auto(self.quantile_left, self.quantile_right)
    if run_gpd:
        self._fit_tail_gpd()
    self._fitted = True
    self.fitted = True
