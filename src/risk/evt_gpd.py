...
class GPD:
    ...
    def _threshold_mrl(
        ...
        pos_data = sorted_data[sorted_data >= 0]
        if len(pos_data) < min_exceedances:
            return max(0.0, np.percentile(data, 90))
        floor_thresh = np.percentile(pos_data, 85)
        pos_upper = pos_data[pos_data >= floor_thresh]
        if len(pos_upper) < min_exceedances:
            pos_upper = pos_data
        
        max_idx = len(pos_upper) - min_exceedances
        if max_idx <= 0:
            return float(pos_upper[0])
        thresholds = pos_upper[np.linspace(0, max_idx, n_thresholds, dtype=int)]
        ...
        if len(valid_mrl) > 2:
            ...
            chosen = valid_thresh[optimal_idx]
            cap = np.percentile(data, 95)
            return min(chosen, cap)
        else:
            return np.percentile(data, 92)
    ...
    def _threshold_stability(
        ...
        chosen = valid_thresh[stable_idx]
        cap = np.percentile(data, 90)
        return min(chosen, cap)
    ...
    def _threshold_automated(
        ...
        pos_vals = sorted_data[sorted_data >= 0]
        floor_auto = np.percentile(pos_vals, 85) if len(pos_vals) > 0 else 0.0
        n_above_p85 = int(np.sum(sorted_data >= floor_auto))
        k_min = max(10, int(np.sqrt(n)))
        k_max = min(int(n * 0.15), n_above_p85 - 10, n - 10)
    ...
    def fit(
        ...
        if threshold < 0.0:
            u_floor = 0.0
            logger.warning(
                f"Threshold {threshold:.4f} < 0 — corrigido para 0.0 "
                f"(threshold negativo incluiria ganhos na cauda de perdas)."
            )
            threshold = u_floor
    ...
    def _fit_mle(self):
        ...
        try:
            ...
            if result.success:
                ...
                if abs(xi_hat - XI_LO) < 1e-4:
                    XI_DIAG_LO = -0.7
                    result2 = optimize.minimize(
                        neg_loglik,
                        x0=[XI_LO - 0.05, sigma0],
                        method='L-BFGS-B',
                        bounds=[(XI_DIAG_LO, XI_HI), (1e-6, None)],
                    )
                    if (
                        result2.success
                        and result2.fun < result.fun * (1 - 1e-4)
                        and result2.x[0] > XI_DIAG_LO + 1e-4
                    ):
                        xi_hat, sigma_hat = result2.x
                        logger.debug(
                            f"Re-otimização confirmou xi={xi_hat:.4f} < {XI_LO} "
                            "com ganho real de verossimilhança."
                        )
                    else:
                        if xi0 <= XI_LO:
                            xi_hat, sigma_hat = XI_LO, result.x[1]
                            logger.debug(
                                f"MLE e PWM ambos querem xi < {XI_LO} "
                                f"(PWM clip={xi0:.4f}). "
                                f"Aceitando MLE no bound xi={XI_LO} com sigma={sigma_hat:.4f}."
                            )
                        else:
                            xi_hat, sigma_hat = xi0, sigma0
                            logger.debug(f"MLE encostou no bound, fallback PWM: xi={xi_hat:.4f}")

                self.xi, self.sigma = xi_hat, sigma_hat
                self.converged = True
            else:
    ...
    def var(self, p: float, n_obs: int) -> float:
        ...
        if not self.converged:
            ...
            logger.warning(
                f"GPD não convergiu — retornando threshold ({self.threshold:.4f}) "
                f"como estimativa conservadora (p={p:.4f}, Fu={Fu:.4f})"
            )
    ...
    # Expected Shortfall (CVaR) analítico via GPD.
    def expected_shortfall(self, p: float, n_obs: int) -> float:
        ...
        if self.xi >= 1:
            logger.warning("ξ >= 1: ES não definido (cauda sem média finita), retornando VaR")
            return var_p

        u = self.threshold
        es = (var_p + self.sigma - self.xi * u) / (1.0 - self.xi)

        if self.xi < 0 and self.upper_endpoint is not None and es > self.upper_endpoint:
            logger.warning(f"ES truncado no upper endpoint: {self.upper_endpoint:.4f}")
            return self.upper_endpoint

        if es < var_p:
            logger.warning(
                f"ES ({es:.4f}) < VaR ({var_p:.4f}) — provavelmente threshold muito alto. "
                f"Retornando VaR como estimativa conservadora."
            )
            return var_p
...
def diagnostic_plots(gpd: GPD, data: np.ndarray, full_analysis: bool = True):
    ...
    residuals = -np.log(1 - theoretical_cdf)
    ...
    if full_analysis:
        ...
        pos_diag = sorted_data[sorted_data >= 0]
        if len(pos_diag) >= min_exc + 1:
            floor_diag = np.percentile(pos_diag, 85)
            pos_upper_diag = pos_diag[pos_diag >= floor_diag]
            if len(pos_upper_diag) < min_exc:
                pos_upper_diag = pos_diag
            max_idx_diag = len(pos_upper_diag) - min_exc
            thresholds = pos_upper_diag[np.linspace(0, max(max_idx_diag, 0), n_thresh, dtype=int)]
        else:
            thresholds = np.maximum(
                sorted_data[np.linspace(0, n_data - min_exc, n_thresh, dtype=int)], 0.0
            )
        xi_vals = []
...
if __name__ == "__main__":
    ...
    comparison = compare_threshold_methods(pos_data, plot=True)
    ...
    gpd = GPD()
    threshold = gpd.select_threshold(pos_data, method='automated', plot=False)
    
    results = gpd.fit(pos_data, threshold, method='mle', use_analytical_hessian=True)
    ...
    diagnostic_plots(gpd, pos_data, full_analysis=True)
    ...
    _xi_sign = '< 0: cauda leve (bounded)' if results_light['xi'] < 0 else '> 0: cauda pesada (Pareto)'
    print(f"ξ = {results_light['xi']:.4f} ({_xi_sign})")
    ...
    n_losses = len(losses_pos)
    
    gpd_losses = GPD()
    threshold_losses = gpd_losses.select_threshold(losses_pos, method='stability', plot=False)
    results_losses = gpd_losses.fit(losses_pos, threshold_losses, method='mle')
    
    print(f"\nAnálise de Perdas (Cauda Negativa):")
    print(f"Threshold: {threshold_losses:.4f}")
    print(f"N exceedances: {results_losses['n_exceedances']} ({results_losses['exceedance_rate']*100:.1f}% das perdas positivas)")
    print(f"ξ = {results_losses['xi']:.4f}")
    print(f"σ = {results_losses['sigma']:.4f}")
    print(f"\nRisco para 1 dia:")
    print(f"VaR 95% (1 dia): {gpd_losses.var(0.05, n_losses):.4f}")
    print(f"VaR 99% (1 dia): {gpd_losses.var(0.01, n_losses):.4f}")
    print(f"ES 99% (1 dia): {gpd_losses.expected_shortfall(0.01, n_losses):.4f}")
