...
class WalkForwardBacktest:
    ...
    def run(
        ...
        for t_idx in rebal_indices:
            ...
            # mesmo GARCH/EVT/quantil por cada um dos ~21 dias do bloco.
...
class RollingBacktestRunner:
    ...
    def run_all(
        ...
        def _run_one(strategy):
            return strategy, self._run_strategy(strategy, returns_df, benchmark_returns)

        max_workers = min(len(self.strategies), max(1, (os.cpu_count() or 2) - 1), 5)
        if len(self.strategies) > 1 and max_workers > 1:
            try:
                with ThreadPoolExecutor(max_workers=max_workers) as executor:
                    futures = {executor.submit(_run_one, strategy): strategy
                               for strategy in self.strategies}
                    for future in as_completed(futures):
                        strategy = futures[future]
                        try:
                            key, res = future.result()
                            self._backtest_results[key] = res
                        except Exception as exc:
                            logger.error(f"Backtest '{strategy}' falhou: {exc}")
            except Exception as exc:
                logger.warning(f"Execução paralela das estratégias falhou ({exc}); usando sequencial.")
                for strategy in self.strategies:
                    try:
                        res = self._run_strategy(strategy, returns_df, benchmark_returns)
                        self._backtest_results[strategy] = res
                    except Exception as exc2:
                        logger.error(f"Backtest '{strategy}' falhou: {exc2}")
        else:
            for strategy in self.strategies:
                try:
                    res = self._run_strategy(strategy, returns_df, benchmark_returns)
