...
for _candidate in (_THIS.parents[1], _THIS.parents[0], _THIS.parent):
...
def estimate_copula(
    ...
    **copula_kwargs,
    ...
    logger.info(
        ...
        f"LL={getattr(copula, 'log_likelihood_', np.nan):.2f}  "
        f"AIC={getattr(copula, 'aic_', np.nan):.2f}  "
        f"BIC={getattr(copula, 'bic_', np.nan):.2f}"
    )
    return U, copula
...
if __name__ == "__main__":
    ...
    try:
        from src.copulas.elliptical import (
