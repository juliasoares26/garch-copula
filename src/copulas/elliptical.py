...
class StudentTCopula:
    ...
    def __init__(
        self,
        method: str = "mle",
        nu_bounds: Tuple[float, float] = (2.0, 100.0),
    ):
        self.method = method
        self.nu_bounds = nu_bounds

        self.R_: Optional[np.ndarray] = None
        self.nu_: float = np.nan
        self.n_dim_: int = 0
        self.n_obs_: int = 0
        self.log_likelihood_: float = np.nan
        self.aic_: float = np.nan
    ...
    def _compute_likelihood(self, U: np.ndarray):
        ...
        k = self.n_dim_ * (self.n_dim_ - 1) // 2 + 1
        self.aic_ = -2 * self.log_likelihood_ + 2 * k
        self.bic_ = -2 * self.log_likelihood_ + k * np.log(T)
...
class BivariateStudentTCopula:
    ...
    def __init__(self, nu_bounds: Tuple[float, float] = (2.0, 100.0)):
        self.rho_: float = 0.0
        self.nu_: float = 5.0
        self.nu_bounds = nu_bounds
        self.log_likelihood_: float = np.nan
